...
float3 EncodeLinearToSrgbColor(float3 Color) {
    ...
    const float3 SqrtColor = sqrt(SaturatedColor);
    const float3 FourthRootColor = sqrt(SqrtColor);
    const float3 EighthRootColor = sqrt(FourthRootColor);
    return saturate((0.662002687f * SqrtColor) + (0.684122060f * FourthRootColor) - (0.323583601f * EighthRootColor) - (0.0225411470f * SaturatedColor));
}
...
float3 ApplyToneMapping(float3 HdrColor) {
    ...
    if (RootConstants.mParameter2 == 0u) {
        ...
        return ApplyGammaCorrection(LinearColor, Gamma);
    }
    ...
    return ApplyGammaCorrection(SaturatedMappedColor, Gamma);
}
