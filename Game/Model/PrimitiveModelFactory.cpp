...
namespace {
    ...
    bool TryParseTerrainHeightSourceTypeText(const std::string& Text, Terrain::TerrainHeightSourceType& OutValue) {
    ...
    bool TryParseTerrainSelector(const std::string& Selector, TerrainSelectorData& OutData) {
        ...
        while (CurrentStart < ParameterText.size()) {
            ...
            if (Key == "HeightSourceType") {
                ...
                OutData.Desc.mHeightSourceType = Terrain::TerrainHeightSourceType::Procedural;
            }
        ...
        if (OutData.Desc.mHeightSourceType == Terrain::TerrainHeightSourceType::Procedural) {
            return true;
        }
    ...
    bool TryCreateTerrainMeshData(const std::string& Selector, MeshData& OutData) {
        ...
        try {
            ...
            Terrain::TerrainMeshBuilder Builder{};
            const Terrain::HeightFieldData HeightField{ HeightFieldFactory.Build(TerrainData.Desc) };
            Terrain::TerrainMeshData TerrainMesh{ Builder.Build(HeightField, TerrainData.Desc) };
